...
!> **Warning broken changes from 2.0.0**: Because we use modular imports,
the `color-` prefix was removed from all color variables in the settings
file, and we add it by the `@forward` directive as alias in the settings
_index_ file: `@forward "settings.colors" as color-*;`
